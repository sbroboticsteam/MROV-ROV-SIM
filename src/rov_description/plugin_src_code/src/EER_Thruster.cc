...
// ---- added (bugfix requirement for multi-path logic) ----
#include <sstream>
#include <vector>
#include <algorithm>
#include <filesystem>
  ...
  EventManager &/*_eventMgr*/)
{
  ...
  if (this->dataPtr->opmode == ThrusterPrivateData::OperationMode::ESCCmd)
  {
    ...
    {
      // ---- bugfix: this must read "thruster_input_voltage", NOT "voltage" ----
      int inputVoltage = _sdf->Get<int>("thruster_input_voltage");

      std::vector<u_char> possibleVoltages = {10, 12, 14, 16, 18, 20};

      this->dataPtr->voltage = *std::min_element(
        possibleVoltages.begin(), possibleVoltages.end(),
    ...
    // ---- bugfix: robust CSV search across colon-separated GZ_SIM_SYSTEM_PLUGIN_PATH ----
    const char *env = std::getenv("GZ_SIM_SYSTEM_PLUGIN_PATH");
    std::string plugin_paths = (env != nullptr) ? std::string(env) : "";
    ...
    std::vector<std::string> behaviorFiles = {
    ...
    const size_t fileIndex = static_cast<size_t>(((this->dataPtr->voltage) - 10) / 2);
    const std::string behaviorFile = behaviorFiles.at(fileIndex);
    ...
    while (std::getline(ss, dir, ':'))
    {
      ...
      const std::string candidate1 = dir + "/plugins/T200_Thruster_Behavior/" + behaviorFile;
      const std::string candidate2 = dir + "/T200_Thruster_Behavior/" + behaviorFile;
    ...
    if (file_path.empty())
    {
      gzerr << "Plugin will not run. Could not find thruster behavior file: "
            << behaviorFile << std::endl;
      gzerr << "Searched using GZ_SIM_SYSTEM_PLUGIN_PATH=" << plugin_paths << std::endl;
      return;
    }
    ...
    while (std::getline(file, current_line))
    {
      ...
      while (current_index < file_row_index && current_index < 256)
      {
        short int difference = file_row_index - current_index + 1;

        this->dataPtr->ESCInputToThrust[current_index] =
            previous_thrust + (current_thrust - previous_thrust) / difference;

        this->dataPtr->ESCInputToAngVel[current_index] =
            previous_angular_velocity + (current_angular_velocity - previous_angular_velocity) / difference;
      ...
      if (file_row_index >= 0 && file_row_index < 256)
      {
      ...
      current_index = file_row_index + 1;
    }

    // Fill remaining indices with last known value (keeps array sane)
    while (current_index < 256)
    {
      this->dataPtr->ESCInputToThrust[current_index] = previous_thrust;
      this->dataPtr->ESCInputToAngVel[current_index] = previous_angular_velocity;
      current_index++;
    }

    file.close();

    // ---- bugfix: cmdMin/cmdMax must be based on actual min/max ----
    auto minmaxIt = std::minmax_element(
      this->dataPtr->ESCInputToThrust.begin(),
      this->dataPtr->ESCInputToThrust.end());

    this->dataPtr->cmdMin = *minmaxIt.first;
    this->dataPtr->cmdMax = *minmaxIt.second;
  ...
  // Everything below this is unchanged from your original file
  // ---------------------------------------------------------
